...
#
# ///////////////     this uses "Hand Command Recognizer on Google AIY Vision Kit" by Dmitri Villevald  https://www.hackster.io/dvillevald/hand-command-recognizer-on-google-aiy-vision-kit-3786f6
#  https://github.com/dvillevald/hand_gesture_classifier
"""
Script to runs MobileNet-based classification model modified so it recognize the chappe gestures.

index  label           function        pin_A pin_B pin_C
0      chappe_in       de/activation   1     1     0
1      chappe_out      de/activation   1     1     0
2      A001            A               0     1     1
3      A002            B               0     0     0
4      A003            C               0     1     0
5      A004            D               0     0     1
...
######### PAS  SUR  QUE  CE  SOIT  NECESSAIRE # Parameters determining size and location of hand box (based on size/location of face box)
#  x_shift_coef=0.0
#  y_shift_coef=1.3
#  scale = 2.0
...
# Print detected chappe gestures
def print_chappe_command(signal):
    if signal == 2:
        print('A')
    elif signal == 3:
        print('B')
        ...
        print('No chappe signal detected')
...
# (2) index = len(labels) otherwise (i.e. none of chappe gestures had a probablity > threshold
...
################## A SUPPRIMER ????     # Error update when determining is face detection is stable
...
################## A SUPPRIMER ????     # Check is face detection is stable
...
###### A SUPPRIMER ?   # Determine location/size of hand box given the location/size of detected face box
...
###### A SUPPRIMER ?   # Select the face with the largest width (others will be ignored)
...
###### A SUPPRIMER ?   # Detect (stable) face
#def detect_face():
...
######  AA  SSUUPPPPRRIIMMEERR      # Determine locations and size of hand box
#def determine_hand_box_params(face_box):
#    face_box = transform(face_box)
#    hand_box_params = hand_box(face_box)
#    return hand_box_params

# Capture raw images into numpy array and crop chappe images
def capture_chappe_image(camera,chappe_box_params):
    chappe_image = []
    image = np.empty((1664 * 1232 * 3,), dtype=np.uint8)
    camera.capture(image, 'rgb')
    image = image.reshape((1232, 1664, 3))
    image = image[:1232, :1640, :]
    chappe_cropped = crop_np(chappe_box_params,image)
    if chappe_cropped:
        chappe_image.append(chappe_cropped)
    return chappe_image

# Classify chappe gestures
def classify_chappe_gestures(img_inference,chappe_images,model,labels,output_layer,threshold):
    for chappe in chappe_images:
        result = img_inference.run(chappe)
...
####### NO #                face_box = detect_face()
####### NO #                hand_box_params = determine_hand_box_params(face_box)
####### NO #                if image_boundary_check(chappe_box_params):
                    break

            # Start chappe classifier
            is_active = False
            ...
            with ImageInference(model) as img_inference:
                while True:
                    ...
                    chappe_image = capture_chappe_image(camera)
                    #,chappe_box_params)
                    output = classify_chappe_gestures(img_inference,chappe_image,model=model,labels=labels,output_layer=args.output_layer,threshold = args.threshold)
                    ...
                    if not is_active:
                        ...
                        # Displaying classified chappe gesture commands
                        if num_short_guess >= (short_buffer_length-1) and is_active:
                            print_chappe_command(short_guess)
